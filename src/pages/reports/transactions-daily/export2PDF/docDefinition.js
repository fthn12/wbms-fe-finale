export default function getDocDefinition(
  ...
  return (function () {
    ...
    const docDefinition = {
      ...
      onlySelected: false,
      columnSeparator: "\t",
      columnKeys: agGridColumnApi.getAllColumns().map((column) => column.colId),
  ...
  function getRowsToExport(columnsToExport) {
    ...
    agGridApi.forEachNodeAfterFilterAndSort((node) => {
      ...
      let rowToExport = columnsToExport.map(({ colId }) => {
        ...
        let tableCell = createTableCell(node, cellValue, colId);
        return tableCell;
      });
  ...
  // }
  function createTableCell(node, value, colId) {
    let cellValue = value !== undefined ? value : "";
    ...
    if (colDef.valueFormatter) {
      ...
      cellValue = colDef.valueFormatter(valueFormatterParams);
    }
    const tableCell = {
      text: cellValue,
      style: "tableCell",
